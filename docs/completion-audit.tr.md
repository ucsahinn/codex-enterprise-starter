...
| Tek komutlu kurulum Codex'i güçlü uzman setup'a çevirmeli. | `scripts/install.ps1 -All -Force` ve `scripts/install.sh --all --force` Codex template'lerini, doğrulanmış public skill source'larını, Git guard'larını, uzman ajanları, profilleri, kuralları ve yerel plugin'i kuruyor. | Tamam |
