...
| Full setup should be idempotent and fail closed. | `scripts/install.ps1` and `scripts/install.sh` skip already installed global skills, install only to the Codex agent target, use an OpenSSL Git override for skill clones, and fail if the Skills CLI reports clone/install/write failures. | Complete |
